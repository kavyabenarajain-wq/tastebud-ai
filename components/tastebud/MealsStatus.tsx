...
export function MealsStatus() {
        ...
        {isFree ? (
          <>
            <p className="text-[13px] font-medium text-ink">
              That&rsquo;s your {snap.trial.images} free {snap.trial.images === 1 ? "image" : "images"} — and they already look like your brand.
            </p>
            <p className="mt-1 text-[12px] leading-relaxed text-muted">
              Keep the studio going. {rec.label} is {rec.monthlyMeals} images a month for ${rec.priceUSD} — every shoot
              on-brand, delivered at 4K. Or grab {pack.meals} images for ${pack.priceUSD}, no subscription.
            </p>
          </>
        ) : (
          <>
    ...
    return (
      <div className="mx-5 mb-2 flex items-center gap-3 rounded-card border border-hairline bg-canvas px-3.5 py-2.5">
        <span className="flex-1 text-[12px] leading-relaxed text-ink">
          {isFree ? (
            <>
              <span className="font-medium">{left} of {snap.trial.images} free {snap.trial.images === 1 ? "taste" : "tastes"} left.</span>{" "}
  ...
  return (
    <div className="mx-5 mb-2 flex items-center gap-2 px-1 text-[11px] text-muted">
      ...
      <span>
        {isFree && snap.trial.active ? `${left} of ${snap.trial.images} free tastes left` : `${left} Meals left`}
      </span>
